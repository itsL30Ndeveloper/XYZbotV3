...
const starts = async (hexa = new WAConnection()) => {
    ...
    hexa.browserDescription = [ 'XYZteam', 'Chrome', '3.0' ]
    console.log(banner.string)
    hexa.on('qr', () => {
        console.log(color('[','white'), color('!','red'), color(']','white'), color(' Scan Bro'))
    })

    fs.existsSync('./session.json') && hexa.loadAuthInfo('./session.json')
    hexa.on('connecting', () => {
        start('2', 'Memuat Koneksi...')
    })
    hexa.on('open', () => {
    ...
    hexa.on('chat-update', async (message) => {
        require('./thisleonreal.js')(hexa, message)
    })
}
